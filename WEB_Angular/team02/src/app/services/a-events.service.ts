...
export class AEventsService {
  ...
  save(aEvent: AEvent): void {
    ...
    // @ts-ignore
    // tslint:disable-next-line:triple-equals
    // replace the AEvent with the same id with the provided return the old, replace aEvent add the new aEvent if none existed and return null

    this.aEvents[this.aEvents.findIndex((event) => event.id == aEvent.id)] = aEvent;
  }

  deleteById(eId: number): AEvent {
    // remove identified aEvent from the collection and return the remove instance, return null if none existed
    // tslint:disable-next-line:triple-equals
